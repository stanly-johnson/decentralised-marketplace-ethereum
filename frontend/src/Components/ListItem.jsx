...
class ListItem extends Component {
  ...
  onSubmit = (event) => {
    ...
    ipfs.add(this.state.buffer, (error, result) => {
      ...
      this.marketInstance
        .createItem(this.state.fileName, this.state.filePrice, String(result[0].hash), { from: this.state.account })
        .then((result) => {
          console.log(result);
        });
    });
  ...
  // function to withdraw the seller balance
  withdrawBalance() {
    this.marketInstance
      .withdrawBalance({ from: this.state.account })
      .then((result) => {
        console.log(result);
            ...
            <div class="col-lg-12 text-center">
              ...
              {this.state && !this.state.loading && (
                <div className="col-md-6">
                  <p>
                    Available balance : {this.state.seller_balance} :{' '}
                    <button class="btn btn-info btn-sm" onClick={() => this.withdrawBalance()}>
                      Withdraw Balance
                    </button>
                  </p>
                  <br />
